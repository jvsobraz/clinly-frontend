...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DatePipe, RouterLink, MatCardModule, MatIconModule, MatButtonModule, MatProgressSpinnerModule, TranslateModule],
  template: `
    <div class="max-w-2xl mx-auto p-6 space-y-6">
      <div class="bg-indigo-600 text-white rounded-2xl p-6">
        <p class="text-indigo-200 text-sm">{{ 'portal.welcome' | translate }}</p>
        <h1 class="text-2xl font-bold">{{ user()?.name }}</h1>
      </div>

      <div>
        <h2 class="font-semibold text-gray-800 mb-3 flex items-center gap-2">
          <mat-icon class="text-indigo-500">upcoming</mat-icon> {{ 'portal.upcoming' | translate }}
        </h2>
        @if (loading()) { <mat-spinner diameter="32" /> }
        @if (!loading() && upcoming().length === 0) {
          <mat-card class="p-4 text-center text-gray-400 text-sm">{{ 'portal.noUpcoming' | translate }}</mat-card>
        }
          ...
          <mat-card class="mb-3">
            <mat-card-content class="p-4 flex items-center gap-4">
              ...
              <span class="px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">
                {{ apt.status === 'Confirmed' ? ('portal.status.confirmed' | translate) : ('portal.status.pending' | translate) }}
              </span>
            </mat-card-content>
          </mat-card>
        }
      ...
      <a routerLink="/patient/appointments" mat-stroked-button class="w-full">
        <mat-icon>history</mat-icon> {{ 'portal.viewHistory' | translate }}
      </a>
    </div>
  `
})
