...
@Component({
  ...
  imports: [
    ...
    MatSelectModule, MatProgressSpinnerModule, TranslateModule,
  ],
  template: `
    <h2 mat-dialog-title>{{ 'appointments.dialog.title' | translate }}</h2>
    <mat-dialog-content>
      @if (loadingData()) {
        ...
        <form [formGroup]="form" class="flex flex-col gap-3 pt-2 min-w-[480px]">
          <mat-form-field appearance="outline">
            <mat-label>{{ 'appointments.dialog.patient' | translate }}</mat-label>
          ...
          <mat-form-field appearance="outline">
            <mat-label>{{ 'appointments.dialog.professional' | translate }}</mat-label>
          ...
          <mat-form-field appearance="outline">
            <mat-label>{{ 'appointments.dialog.service' | translate }}</mat-label>
          ...
          @if (selectedServiceDuration()) {
            <p class="text-sm text-slate-500 -mt-1 px-1">
              {{ 'appointments.dialog.duration' | translate }}: <span class="font-medium text-slate-700">{{ selectedServiceDuration() }} min</span>
            </p>
          }

          @if (rooms().length > 0) {
            <mat-form-field appearance="outline">
              <mat-label>{{ 'appointments.dialog.room' | translate }}</mat-label>
              <mat-select formControlName="roomId">
                <mat-option [value]="null">{{ 'appointments.dialog.noRoom' | translate }}</mat-option>
          ...
          <mat-form-field appearance="outline">
            <mat-label>{{ 'appointments.dialog.datetime' | translate }}</mat-label>
            <input matInput type="datetime-local" formControlName="scheduledAt" />
          </mat-form-field>

          <mat-form-field appearance="outline">
            <mat-label>{{ 'appointments.dialog.notes' | translate }}</mat-label>
    ...
    <mat-dialog-actions align="end" class="gap-2 px-6 pb-4">
      ...
      <button mat-flat-button [disabled]="form.invalid || saving() || loadingData()" (click)="save()">
        {{ saving() ? ('appointments.dialog.booking' | translate) : ('appointments.dialog.book' | translate) }}
      </button>
    </mat-dialog-actions>
  `,
})
export class AppointmentDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    forkJoin({
      ...
      error: () => {
        this.snackBar.open(this.translate.instant('common.loading'), 'X', { duration: 4000 });
        this.loadingData.set(false);
      },
    });
  }
  ...
  save() {
    ...
    this.appointmentService.create(this.data.tenantId, body).subscribe({
      next: result => {
        ...
        this.snackBar.open(this.translate.instant('common.save'), 'X', { duration: 3000 });
